    ...
    public class StudentsResults
    {
        public static void Main()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                var advCGrade = double.Parse(studentArgs[1]);
                var cOOPGrade = double.Parse(studentArgs[2]);
                var advancedOOPGrade = double.Parse(studentArgs[3]);
                var averageGrade = (advCGrade + cOOPGrade + advancedOOPGrade) / 3;

                Console.WriteLine(string.Format("{0, -10}|{1,7:f2}|{2,7:f2}|{3,7:f2}|{4,7:f4}|", studentName, advCGrade, cOOPGrade, advancedOOPGrade, averageGrade));
            }
        }
    }
}
